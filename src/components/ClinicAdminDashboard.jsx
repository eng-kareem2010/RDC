...
export default function ClinicAdminDashboard() {
    ...
    const executeAdminLogin = async (e) => {
        ...
        if (data) {
            ...
            toast.error('البيانات التي أدخلتها غير مسجلة من فضلك قم بالمحاولة مرة أخرى ✖');
        }
    };
    ...
    const createNewShiftSlots = async (e) => {
        ...
        if (!error) {
            toast.success('تم توليد المواعيد!');
            window.location.reload();
        } else {
            toast.error('فشل في توليد الساعات.');
        }
    };
    ...
    return (
        <div className="dentist-app-body p-6" dir="rtl">
            <div className="molar-gradient-top p-6 mb-8 text-white rounded-xl">
                ...
                <button onClick={() => { sessionStorage.clear(); window.location.reload(); }} className="mt-4 bg-rose-500 p-2 px-4 rounded-lg">خروج</button>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                {/* عدل الجزء الخاص بالجدول في الـ Admin Dashboard ليصبح كالتالي */}

                <div className="clinical-card-shell xl:col-span-2 overflow-x-auto"> {/* أضفنا overflow-x-auto هنا */}
                    ...
                    {/* الجدول هنا داخل الـ div اللي بيعمل سكرول */}
                    <table className="w-full text-right min-w-[600px]"> {/* أضفنا min-w عشان الجدول ميتمطش زيادة */}
                        <thead>
                        ...
                        <tbody>
                            {bookings.map(book => (
                                <tr key={book.id} className="border-b text-sm">
                                    <td className="p-3 whitespace-nowrap">{book.patient_name}</td>
                                    <td className="p-3 whitespace-nowrap">{book.phone}</td>
                                    <td className="p-3 whitespace-nowrap">{book.specialty}</td>
                                    <td className="p-3 font-bold text-sky-700 whitespace-nowrap">
                                        {(() => {
                                            const slot = allClinicSlots.find(s => s.id === book.slot_id);
                                            if (!slot) return "غير متاح";
                                    ...
                                    <td className="p-3">
                                        <button onClick={() => deleteBooking(book.id)} className="text-white bg-rose-500 px-3 py-1 rounded-lg text-xs whitespace-nowrap">حذف</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
