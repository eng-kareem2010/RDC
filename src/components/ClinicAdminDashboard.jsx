...
export default function ClinicAdminDashboard() {
    ...
    useEffect(() => {
        ...
        async function getAdminData() {
            ...
            if (bookingData && slotData) {
                // فلترة يدوية للحجوزات التي تطابق التاريخ المختار
    ...
    const createNewShiftSlots = async (e) => {
        ...
        while (current < finish) {
            const startStr = current.toTimeString().split(' ')[0];
            current.setMinutes(current.getMinutes() + 30);
            const endStr = current.toTimeString().split(' ')[0];
            if (new Date(`${newShiftDate}T${startStr}`) < finish) {
        ...
        if (!error) {
            toast.success('تم توليد المواعيد بنجاح!');
    ...
    const deleteBooking = async (id) => {
        if (!window.confirm('هل أنت متأكد من إلغاء حجز هذا المريض؟')) return;
    ...
    return (
        <div className="dentist-app-body p-6" dir="rtl">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="clinical-card-shell h-96 overflow-y-auto">
                    ...
                    {allClinicSlots.map(slot => (
                        <div key={slot.id} className="flex justify-between p-2 border-b text-sm">
                            <span>{slot.slot_date} - {slot.start_time} ({slot.specialty})</span>
                ...
                {/* جدول المرضى */}
                <div className="clinical-card-shell xl:col-span-3">
                    ...
                    <table className="w-full text-right">
                        <thead>
                            <tr className="bg-slate-50 text-slate-600 text-xs font-bold">
                                ...
                                <th className="p-3">الساعة المحجوزة</th>
                                ...
                                <tr key={book.id} className="border-b text-sm">
                                    ...
                                    <td className="p-3 font-bold text-sky-700">
                                        {(() => {
                                            ...
                                            hours = hours % 12 || 12; // تحويل الـ 0 لـ 12
